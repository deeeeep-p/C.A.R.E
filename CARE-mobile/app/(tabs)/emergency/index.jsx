...
export default function EmergencyRecordingApp() {
  ...
  const [messages, setMessages] = useState([
    "Welcome to Emergency Recording App",
  ]);
  ...
  async function transcribeAudio(audioUri) {
    try {
      ...
      const url =
        process.env.FLASK_URL || "http://192.168.31.116:5005/transcribe";
  ...
  async function processEmergency() {
    if (!transcript || !location || !clientId || wsStatus !== "Connected") {
      console.log(
        "Cannot process: Ensure transcription is complete and WebSocket is connected.",
        "warning"
      );
      return;
    }
    ...
    try {
      const response = await fetch("http://192.168.31.116:3000/emergency", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(
          `Error queuing request: ${responseData.error || response.statusText}`,
          "error"
        );
      }
    } catch (error) {
      ...
      console.log(
        `Failed to send emergency request: ${error.message}`,
        "error"
      );
    } finally {
      setStatus("idle");
  ...
  const connectWebSocket = () => {
    const wsUrl = "ws://192.168.31.116:3000";
  ...
  const handleProcessingResult = (result) => {
    ...
    if (result.transcript_analysis) {
      ...
      if (analysis.location_mentioned)
        addMessage(`Location: ${analysis.location_mentioned}`, "info");
      if (analysis.depts)
        addMessage(`Departments: ${analysis.depts.join(", ")}`, "info");
      if (analysis.key_issues)
        addMessage(`Key Issues: ${analysis.key_issues.join(", ")}`, "info");
    }
                ...
                <View className="mt-10 bg-slate-800/90 rounded-3xl p-6 shadow-xl border border-gray-800">
                  ...
                  <View className="space-y-6 mt-4">
                    <View className="mb-4 bg-slate-800/80 p-5 rounded-xl border border-slate-700">
                      <Text className="text-blue-400 text-base font-medium mb-2">
                        Summary
                      </Text>
                    ...
                    <View className="mb-4 bg-slate-800/80 p-5 rounded-xl border border-slate-700">
                      ...
                      <Text className="text-white text-xl">
                        {Array.isArray(services.key_issues)
                          ? services.key_issues.join(" • ")
                          : services.key_issues}
                      </Text>
                    </View>

                    {Array.isArray(services.depts) &&
                    services.depts.length > 0 ? (
                      <View className="mb-4 bg-slate-800/80 p-5 rounded-xl border border-slate-700">
